...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://pastebin.com/raw/2WqGJVEx').text
			if basesplit in plr:
				key = basesplit
...
\033[1;92m================================================================
  \033[1;92m[â√š] \033[1;92mDEVOLPER    :    MD REJAUL KARIM
  \033[1;92m[â√š] \033[1;92mFACEBOOK    :    MD REJAUL KARIM
  \033[1;92m[â√š] \033[1;92mWHATSAPP    :    01716813329
  \033[1;92m[â√š] \033[1;92mGITHUB      :    broken-hart
  \033[1;92m[â√š] \033[1;92mTOOLS       :    2008-2011 CLONER
\033[1;92m================================================================""")
		print("%s [%s🌻%s] %sTOOL NAME : %sUID CLON"%(G,R,G,G,G))
		print("%s [%s🌻%s] %sVERSION   : %s1.1"%(G,R,G,G,G))
		print("%s [%s🌻%s] %sSTATUS  : %s%s"%(G,R,G,G,G,stat))
		print("%s [%s🌻%s] %sYOR KEY    : %s"%(G,R,G,G,key)) 
		print("\n\x1b[1;932=================================================================  ")
		print("%s [%s1%s]%s RANDOM FB ID 2004-5 %s(PRO) V1"%(R,C,R,G,G))
		print("%s [%s2%s]%s RANDOM FB ID 2004-5 %s(PRO) V2"%(R,C,R,G,G))
		print("%s [%s3%s]%s RANDOM FB ID 2004 %s(PRO) V3"%(R,C,R,G,G))
		print("%s [%s4%s]%s FROM EMAILS %s(PRO)"%(R,C,R,G,G))
		print("%s [%s5%s]%s RANDOM FB ID Custom %s(PRO) V1"%(R,C,R,G,G))
		print(GET)
		hoga = input("\n%s [?] CHOICE : "%(G))
		if hoga in ["", " "]:
			Main()
	...
	def fbtua(self):
		...
		xx = 999999999
		idx = "100000"
		limit = int(input("\033[1;92m [+] ENTER LIMIT \033[1;92m(100000 MAX): \033[1;92m"))
		if (limit)>100000:
	...
	def old_9(self):
		...
		xx = 999999
		idx = "100000000"
		limit = int(input("\033[0;92m [+] ENTER LIMIT \033[0;92m(100000 MAX): \033[0;92m"))
		if (limit)>100000:
			exit("\n%s [!] DON'T CROSS THE LIMIT BRO :)"%(R))
		try:
			for n in range(limit):
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,P))
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;92m%s\033[0;92m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR REJAUL  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,P))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def old4_6(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;92m%s\033[0;92m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR RRJAUL "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,P)) 
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:REJAUL): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;92m[RNL] :-\033[0;92m%s/%s -\033[0;92m[OK:%s] \033[0;94m[CP:%s]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
